...
// GET - Search customers with infinite scroll
export async function GET(request: NextRequest) {
  try {
    ...
    params.set("fields[1]", "full_name")
    params.set("fields[2]", "phone_number")

    // Filter by search query
    if (search) {
      params.set("filters[$or][0][full_name][$containsi]", search)
      params.set("filters[$or][1][phone_number][$containsi]", search)
    }

    // Dùng REST API
    const url = `${BASE_URL}/api/customers?${params.toString()}`
